...
/*package*/ final class Out {
     ...
     *
     * @param level - {@link OutLevel} the log statement comes from
     * @param str - log string to format
     * @param args -varArg params for formatting
     */
    static void log(OutLevel level,String str,Object... args) {
        if (slf4j) {
            ...
            final Logger logger = (clazz == null) ? defaultLog : logMap.getOrDefault(clazz, defaultLog);
            final String logStr = String.format(str, args);
            switch (level) {
            case TRACE:
                logger.trace(logStr);
                break;
            ...
            final String logStr = String.format(timeStr() + level.levelString() + str, args);
            switch (level) {
            case ERROR:
